...
Route::middleware(['web'])->group(function () {
    ...
    // Route untuk debug reCAPTCHA (Simple & Safe)
    Route::get('/debug/recaptcha', function () {
        if (!request()->has('key') || request('key') !== 'ganesha2024deploy') {
            return response()->json(['error' => 'Unauthorized'], 403);
        }

        // Simple debug info without complex try-catch
        $debug = [
            'app_name' => config('app.name'),
            'env_app_name' => env('APP_NAME'),
            'app_env' => app()->environment(),
            'recaptcha_site_key' => env('RECAPTCHA_SITE_KEY') ? substr(env('RECAPTCHA_SITE_KEY'), 0, 20) . '...' : 'NOT_SET',
            'recaptcha_secret_key' => env('RECAPTCHA_SECRET_KEY') ? substr(env('RECAPTCHA_SECRET_KEY'), 0, 20) . '...' : 'NOT_SET',
        ...
        if (request()->has('test_api')) {
            ...
            if ($siteKey && $secretKey) {
                ...
                $debug['api_test'] = $response->json();
            } else {
                $debug['api_test'] = ['error' => 'Keys not configured'];
            }
        }
